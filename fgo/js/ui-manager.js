...
class UIManager {
    ...
    `;
            ...
            cardElement.addEventListener('click', () => {
                this.showCardDetail(card);  // 直接展示详情
            });
    ...
    showCardDetail(card, count = 1) {
        ...
        // 使用角色的图片(image)而不是职阶的图片(class_image)
...
async showPullAnimation(card) {
    ...
    this.animatedCardEl.innerHTML = `
        ...
        <img class="card-image" src="${card.class_image}" alt="Card Back"> <!-- 初始显示卡背 -->
    `;
    ...
    await new Promise(resolve => {
        setTimeout(() => {
            this.animatedCardEl.classList.add('rotate-card');  // 执行旋转
            resolve();
        }, 1000);  // 延时1秒再开始旋转
    });

    // 旋转结束后，切换到卡片正面
    const cardImage = this.animatedCardEl.querySelector('.card-image');
    cardImage.src = card.image;  // 切换到卡片的正面图像
    cardImage.style.objectFit = 'contain';  // 确保图片自适应容器大小
    cardImage.style.width = '100%';  // 图片宽度填满容器
    cardImage.style.height = '100%'; // 图片高度填满容器

    // 等待点击事件展示卡片详情
    this.animatedCardEl.addEventListener('click', () => {
        this.showCardDetail(card);  // 用户点击后展示详情
    });
